...
- **A-focus-remote.html** (Option A): dashboard is ambient/decorative; click **Open Focus Remote** → a 420×680 window (backed by `A-focus-remote-window.html`, which also opens standalone) lists sessions by urgency; click **FOCUS ▶** to simulate the terminal switch. Footer link previews the all-clear empty state.
