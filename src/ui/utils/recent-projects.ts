 ...
 */
export function selectRecentProjects(projects: ProjectSnapshot[], limit: number): ProjectSnapshot[] {
  if (projects.length === 0) return []
  const effectiveLimit = Number.isFinite(limit) && limit >= 1 ? Math.floor(limit) : projects.length
  const ranked = [...projects].sort((a, b) => activityOf(b) - activityOf(a))
