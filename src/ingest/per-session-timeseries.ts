...
export function derivePerSessionTimeSeries(opts: {
	...
	const result = withReadonlyDb(opts.sqlitePath, (db) => {
		// Step 1: Get top-level sessions matching this project directory.
		// Child/background sessions are excluded to match session summary inclusion.
		const sessionRows = db
			.query(
				"SELECT id, title, directory, parent_id FROM session WHERE directory IS NOT NULL",
			)
			.all() as Array<{
			id: unknown;
			title: unknown;
		...
		for (const row of sessionRows) {
			...
			if (typeof row.parent_id === "string" && row.parent_id) continue;
		...
		for (const row of messageRows) {
			if (typeof row.id !== "string" || typeof row.session_id !== "string")
				continue;
			const createdAt =
				typeof row.time_created === "number" ? row.time_created : null;
		...
		for (const row of partRows) {
			if (typeof row.message_id !== "string" || typeof row.data !== "string")
				continue;
			let parsed: unknown;
			try {
			...
			toolCountByMessage.set(
				row.message_id,
				(toolCountByMessage.get(row.message_id) ?? 0) + 1,
			);
		}
		...
		for (const [sessionId, values] of sessionBuckets) {
			...
			const isBackground =
				boulderSet.size > 0 ? !boulderSet.has(sessionId) : false;
